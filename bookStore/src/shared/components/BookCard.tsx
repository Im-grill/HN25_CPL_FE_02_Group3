...
export default function BookCard({props}: BookCardProps) {
    return (
        ...
        <div
            className='relative z-0  rounded-2xl bg-white shadow-black/20 hover:shadow-xl cursor-pointer flex flex-col justify-between  h-full '>
            ...
            <div className='mt-2 ml-3  items-baseline hidden md:flex'>
                {props.current_seller?.price !== undefined && (
                    <>
                        <span
                            className='text-[18px] text-[#FF424E] font-semibold'>{props.current_seller.price.toLocaleString('vi-VN')}</span>
                        ...
                        (((props.original_price - props.current_seller.price) / props.original_price) * 100) > 0 ?
                            <span
                                className='font-medium text-[14px] ml-1 self-center bg-black/5 rounded-xl p-1 '>-{`${(((props.original_price - props.current_seller.price) / props.original_price) * 100).toFixed(0)}`}%</span> : ""}
                    </>
                )}
            </div>
            {/* author */}
                        ...
                        <>
                            ...
                            <div className='flex items-center'>
                                {props.original_price !== undefined && (((props.original_price - props.current_seller.price) / props.original_price) * 100) > 0 ?
                                    <span
                                        className='font-medium text-[14px]   bg-black/5 rounded-xl p-1 '>-{`${(((props.original_price - props.current_seller.price) / props.original_price) * 100).toFixed(0)}`}%</span> : ""}
                                {props.original_price !== undefined && (((props.original_price - props.current_seller.price) / props.original_price) * 100) > 0 ?
                                    <div className='items-baseline flex ml-2 '>
                                        <span
                                            className='font-medium text-[10px] text-[#808089] line-through'>{props.original_price.toLocaleString("vi-VN")}</span>
            ...
            <div className='ml-3 mr-3 border-t-1 border-[#EBEBF0]  pt-2 pb-2 flex items-center mt-5'>
                {
                    props.fastShip ? <>
                            <img
                                src='https://s3-alpha-sig.figma.com/img/9f63/2df5/52d4ff178b5e56072899664c52a61fe5?Expires=1745193600&Key-Pair-Id=APKAQ4GOSFWCW27IBOMQ&Signature=L07qhIWxsoESr22Fhy-h3W3JlynDVcIz5s8Oz8Nkrf-2t8W2AX2WIF7iBCdurqcEoiXadvU902oF7u2--X38P61RecGX2d-UM-o7vHuEtLjQPB-gOHEVRQ0pyq5aMyzQDyhja~Eqcx7XD-Eie61PwF6ENOg91o2Vhuuqn0kRbo1J9yuPUP-pPoAF-42yUuV2Lw66GrooiHlmyTEDmIlOsy271YCcCvDP2-YcQuyfhus-NdfR7BwtVHAtMukisUOSsAw~O3O-t0L4et25PKRy~NouPFNTcUr9fg3y~C0Qy2nk8DJ~lsnKn45iheEGGM7W~a~BgZ806De4Gsg3HnjC0Q__'
                                className='h-4 w-8 mr-2' alt=''/>
